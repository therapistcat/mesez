...
function splitUtf8ByByteWindow(input: string): string[] {
  ...
  while (cursor < input.length) {
    ...
    while (end < input.length) {
      ...
      if (consumedBytes + charBytes > MAX_FRAGMENT_BYTES && consumedBytes > 0) break
      if (consumedBytes + charBytes > targetBytes && consumedBytes >= MIN_FRAGMENT_BYTES) break

      consumedBytes += charBytes
      end += char.length

      if (consumedBytes >= targetBytes) break
    }
...
export async function fragmentMessage(
  message: { to: string } & Record<string, unknown>
): Promise<FragmentPacket[]> {
  const to = typeof message.to === 'string' ? message.to.trim() : ''
  if (!to) throw new Error('Message recipient is required for fragmentation')
